...
class mitm_t : public ost::TCPSocket, public ost::Thread {
  uint cid;
  data_cb_t app_tx, app_rx;
  bool app_tx_cb(uint cid_, std::string &d) { return app_tx(cid_, d); }
  bool app_rx_cb(uint cid_, std::string &d) { return app_rx(cid_, d); }
 public:
  mitm_t(u16 p, data_cb_t app_tx_, data_cb_t app_rx_) : TCPSocket(ost::IPV4Address("0.0.0.0"), p),
    cid(0), app_tx(app_tx_), app_rx(app_rx_) { }
  void run() {
    while(1) {
